...
export function EthnicityDetailClient({ ethnicity }: { ethnicity: any }) {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground selection:bg-heritage-turquoise/30 font-sans">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 flex justify-between items-center backdrop-blur-md bg-white/60 border-b border-black/5 shadow-sm">
        <Link href="/" className="flex items-center text-sm font-sans uppercase tracking-widest font-bold text-slate-600 hover:text-heritage-turquoise transition-colors group">
          <ArrowLeft className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform" />
          QUAY LẠI
        </Link>
        <div className="text-xl font-heading font-bold text-slate-900 tracking-tight">
      ...
      <section className="relative h-[80vh] flex items-end pb-24 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full">
          ...
          >
            ...
            <h1 className="text-7xl md:text-9xl font-heading font-bold text-slate-900 mb-6 tracking-tight">
              {ethnicity.name}
            </h1>
            {ethnicity.population && (
                <div className="flex flex-col md:flex-row md:items-center text-slate-600 text-xs font-sans uppercase tracking-[0.2em] space-y-2 md:space-y-0 md:space-x-8">
                    <div className="flex items-center">
                        <Users className="w-4 h-4 mr-2 text-national-gold" />
                        {ethnicity.population.provinceStats}
                    </div>
                    {ethnicity.population.details && (
                        <div className="flex items-center text-heritage-turquoise border-l border-slate-200 md:pl-8">
                            {ethnicity.population.details}
                        </div>
                    )}
                </div>
      ...
      <section className="py-24 px-6 relative">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
            ...
            >
              <h2 className="text-3xl font-heading font-bold text-slate-900 mb-10 border-l-4 border-heritage-turquoise pl-6 uppercase tracking-wider">
                TẬP TỤC & VĂN HÓA
              </h2>
              <div className="text-xl text-slate-700 leading-relaxed font-serif space-y-8">
                ...
                {ethnicity.ecologicalLogic && (
                    <div className="bg-slate-50 border-l-2 border-heritage-turquoise p-8 rounded-r-3xl my-12 relative overflow-hidden group shadow-sm">
                        ...
                        <h4 className="flex items-center text-heritage-turquoise font-bold uppercase text-xs tracking-widest mb-4">
                            <Leaf className="w-4 h-4 mr-2" /> Trí tuệ sinh thái
                        </h4>
                        <p className="text-slate-800 text-lg italic relative z-10">{ethnicity.ecologicalLogic}</p>
                    </div>
                )}
                ...
                {ethnicity.culturalNarrative && (
                    <div className="py-12 border-y border-slate-100 my-12">
                        <blockquote className="text-2xl font-serif italic text-slate-700 border-l-4 border-national-gold pl-8">
                            "{ethnicity.culturalNarrative}"
                        </blockquote>
                    </div>
                )}
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white border border-slate-200 p-10 rounded-3xl shadow-xl relative overflow-hidden">
                ...
                {ethnicity.costumeInfo.jewelryNote && (
                    <div className="mb-8 p-6 bg-slate-50 rounded-2xl border-l-2 border-national-gold">
                        <div className="text-[10px] text-national-gold font-bold mb-2 uppercase tracking-widest">Ghi chú trang sức</div>
                        <p className="text-xs text-slate-600 italic leading-relaxed">{ethnicity.costumeInfo.jewelryNote}</p>
                    </div>
                )}
                ...
                {ethnicity.costumeInfo.funeralNote && (
                    <div className="mb-8 p-6 bg-red-500/5 rounded-2xl border-l-2 border-red-500/50">
                        ...
                        <p className="text-xs text-slate-600 italic leading-relaxed">{ethnicity.costumeInfo.funeralNote}</p>
                    </div>
                )}

                <div className="flex space-x-3 pt-4 border-t border-slate-100">
                  {ethnicity.costumeInfo.colors.map((color: string, i: number) => (
                    <div
                      key={i}
                      className="w-10 h-10 rounded-full border border-slate-200 shadow-inner group relative"
              ...
              {ethnicity.ritualPhilosophy && (
                <motion.div 
                    ...
                    className="bg-heritage-turquoise/5 border border-heritage-turquoise/20 p-10 rounded-3xl relative overflow-hidden shadow-xl"
                >
                  <Sparkles className="absolute bottom-4 right-4 text-heritage-turquoise/10 w-16 h-16" />
                  <span className="inline-block text-[10px] bg-heritage-turquoise text-white font-bold px-3 py-1 rounded-full mb-6 uppercase tracking-widest">
                    {ethnicity.id === 'o-du' ? 'Cảm thức thời gian' : 'Triết lý Tâm linh'}
                  </span>
                  <h3 className="text-slate-900 text-xl font-bold mb-4 uppercase tracking-tighter">
                    {ethnicity.id === 'o-du' ? 'TIẾNG SẤM GỌI MÙA' : 'Tín ngưỡng & Phong tục'}
                  </h3>
                  <p className="text-slate-600 text-sm leading-relaxed italic">{ethnicity.ritualPhilosophy}</p>
                </motion.div>
              )}
            </div>
          </div>
          ...
          {ethnicity.heritage && ethnicity.heritage.length > 0 && (
            <div className="mt-32">
              <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                <div>
                  <h2 className="text-4xl md:text-6xl font-heading font-bold text-slate-900 mb-4">
                ...
                <div className="h-px flex-1 bg-slate-100 hidden md:block mb-4 ml-8" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {ethnicity.heritage.map((item: any) => (
      ...
      {ethnicity.id === "tho" && (
        <section className="py-24 border-t border-white/5">
          <div className="max-w-7xl mx-auto px-6">
            <h2 className="text-5xl font-heading font-bold text-slate-900 mb-16 px-6 flex items-center">
              <Sparkles className="text-national-gold mr-6 w-10 h-10" /> Truyền thuyết & Tiếp biến
            </h2>
            <ClothingComparison />
            <div className="mt-24">
                <h3 className="text-3xl font-heading font-bold text-slate-900 mb-16 px-6 flex items-center">
      ...
      {ethnicity.cuisines && ethnicity.cuisines.length > 0 && (
        <section className="py-32 px-6 border-t border-slate-100 bg-slate-50/50">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center mb-20 space-y-4 md:space-y-0">
               ...
               <div className="space-y-2">
                 <h2 className="text-5xl md:text-7xl font-heading font-bold text-slate-900">Ẩm thực Di sản</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              {ethnicity.cuisines.map((cuisine: any) => (
                <div key={cuisine.id} className="group relative overflow-hidden rounded-[3.5rem] bg-white border border-slate-200 hover:border-heritage-turquoise/30 transition-all flex flex-col md:flex-row shadow-xl">
                  ...
                  <div className="p-12 md:w-3/5 flex flex-col justify-center bg-white/40 backdrop-blur-md">
                    <h3 className="text-3xl font-heading font-bold text-slate-900 mb-6 underline decoration-heritage-turquoise/30 underline-offset-8">
                        {cuisine.name}
                    </h3>
                    <p className="text-slate-600 font-serif text-lg leading-relaxed mb-8 italic">
                        "{cuisine.description}"
                    </p>
                    {cuisine.ingredients && (
                      <div className="flex flex-wrap gap-3 mt-auto">
                        {cuisine.ingredients.map((ing: string, idx: number) => (
      ...
      {ethnicity.id === "o-du" && (
        <section className="py-32 bg-background border-t border-slate-100">
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex flex-col items-center text-center mb-24 space-y-6">
               <div className="p-6 bg-national-gold/10 rounded-full">
                <Music className="text-national-gold w-12 h-12" />
               </div>
               <h2 className="text-5xl md:text-7xl font-heading font-bold text-slate-900 tracking-tight uppercase">Không gian Chăm Phtrong</h2>
               <p className="text-slate-600 max-w-2xl font-serif text-xl italic">"Gột rửa những muộn phiền của năm cũ để mở lòng đón sinh khí thịnh vượng của năm mới tại dòng suối Nậm Ngân."</p>
            </div>
            <SacredThunder />
          </div>
        </section>
      ...
      {/* Footer Branding */}
      <footer className="py-32 px-6 border-t border-slate-100 text-center bg-slate-50 overflow-hidden relative">
        ...
        <div className="relative z-10 space-y-8">
            <div className="flex justify-center items-center space-x-4 mb-4">
                ...
                <span className="text-slate-500 text-xs font-bold uppercase tracking-[0.3em]">Bảo tồn bởi Đội ngũ Di sản Số Nghệ An</span>
            </div>
            <div className="text-slate-500 text-[10px] uppercase tracking-[0.6em] font-bold">
              DI SẢN DÂN TỘC NGHỆ AN | PHIÊN BẢN SỐ 2.0
            </div>
        </div>
      </footer>
    </main>
