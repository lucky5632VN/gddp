...
export function EthnicityGrid() {
  return (
    <section id="ethnicities-grid" className="relative py-24 px-4 bg-slate-950 font-sans">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 px-4">
          ...
          <motion.h2
            ...
            viewport={{ once: true }}
            className="text-4xl md:text-6xl text-white font-heading font-bold mb-6"
          >
        ...
        <motion.div 
          ...
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          className="mb-24 overflow-hidden rounded-3xl border border-white/10 bg-slate-900/40 backdrop-blur-md"
        >
          <div className="p-8 border-b border-white/5 bg-white/5">
             <h3 className="text-white font-bold text-xl">Bảng tổng hợp đối sánh trực quan</h3>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {ethnicities.map((ethnicity) => (
                  <TableRow key={ethnicity.id} className="border-white/5 hover:bg-white/5 transition-colors group">
                    <TableCell className="font-heading font-bold text-white text-lg">{ethnicity.name}</TableCell>
                    <TableCell className="text-slate-400 text-sm py-6">
                      <div className="mb-2 font-bold text-slate-300">{ethnicity.location.join(", ")}</div>
                      <div className="text-xs italic">{ethnicity.population?.provinceStats}</div>
                    </TableCell>
                    <TableCell className="text-slate-300 text-sm leading-relaxed max-w-md">
